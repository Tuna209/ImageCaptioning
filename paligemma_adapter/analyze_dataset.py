...
"""
Script to analyze the RISCM dataset.
"""
...
def analyze_captions(train_df, val_df, test_df):
    ...
    plt.savefig("paligemma_adapter/caption_length_distribution.png")
    plt.close()
    
    # Analyze vocabulary
    all_words = []
    for caption in all_captions:
        all_words.extend(word_tokenize(caption.lower()))
...
def analyze_images(data_dir, train_df, sample_size=100):
    ...
    plt.ylabel("Frequency")
    plt.grid(True, alpha=0.3)
    plt.savefig("paligemma_adapter/image_size_distribution.png")
    plt.close()
...
def main():
    ...
    os.makedirs("paligemma_adapter", exist_ok=True)
    ...
    print("\nAnalysis complete. Results saved to paligemma_adapter/")
