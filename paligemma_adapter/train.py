...
from adapter_config import get_lora_config, get_default_config
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="paligemma_adapter/outputs", help="Directory to save the model")
    ...
    if config["use_wandb"]:
        wandb.init(
            ...
            name=f"paligemma-adapter-{config['adapter_size']}",
            config=config
        )
    ...
    print(f"Using {config['subset_size']} train examples (out of {len(train_dataloader.dataset) * config['batch_size']})")
    print(f"Using {config['val_subset_size']} val examples (out of {len(val_dataloader.dataset) * config['batch_size']})")
    ...
    for epoch in range(config["num_epochs"]):
        ...
        # Print results
        print(f"Epoch {epoch + 1}: Train Loss={train_loss:.4f}, Train PPL={train_perplexity:.2f}; "
