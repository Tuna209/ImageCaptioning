...
from adapter_code.dataset import create_dataloaders
from adapter_code.adapter_config import get_lora_config
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="adapter_code/outputs", help="Directory to save the model")
    ...
    # Initialize wandb
    if config["use_wandb"]:
        # Extract experiment name from output directory if available
        output_dir_parts = config["output_dir"].split("/")
        experiment_name = output_dir_parts[-1] if len(output_dir_parts) > 0 else ""

        # Create wandb run name
        if "benchmark" in config["output_dir"] or "sweep" in config["output_dir"]:
            # For benchmark/sweep runs, use the experiment name from the output directory
            run_name = experiment_name
        else:
            # For manual runs, create a descriptive name
            run_name = f"paligemma-adapter-{config['adapter_size']}-lr{config['learning_rate']:.0e}"

            # Add optimization details
            optimization_details = []

            if config["warmup_ratio"] != 0.1:
                optimization_details.append(f"warmup{config['warmup_ratio']}")

            if config["weight_decay"] != 0.01:
